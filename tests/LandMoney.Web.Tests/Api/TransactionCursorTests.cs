...
public class TransactionCursorTests
{
    ...
    // The length check is what stops a query string full of rubbish being decoded
    // into a buffer the size of whatever was sent. It is not a security control on
    // its own -- the server in front has its own URL limits -- and it is the cheapest
    // possible answer to the one input that costs anything before it is understood.
    [Fact]
    public void A_token_far_too_long_to_be_one_is_refused_unread()
    {
